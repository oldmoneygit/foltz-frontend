...
console.log('🧪 SHOPIFY CSV GENERATOR - TESTE (5 produtos)\n')
...
// Processar cada liga até ter 5 produtos
outerLoop:
for (const leagueId of Object.keys(leaguesData)) {
  ...
  for (const product of league.products) {
    if (productCount >= MAX_PRODUCTS) break outerLoop
    ...
    // IMAGEM PRINCIPAL - Caminho absoluto
    const mainImagePath = product.main_image
      ? path.resolve(__dirname, '..', product.main_image)
      : ''
    ...
    if (mainImagePath && fs.existsSync(mainImagePath)) {
      imageExists = true
      console.log(`✅ ${product.name} - Imagem encontrada: ${path.basename(mainImagePath)}`)
    } else {
      console.log(`⚠️  ${product.name} - Imagem não encontrada: ${mainImagePath}`)
    }
    ...
    productCount++
    console.log(`📦 Produto ${productCount}/5: ${title} (${sizes.length} variantes)\n`)
  }
}
...
// Salvar arquivo
const outputPath = path.join(__dirname, '../shopify-products-TEST.csv')
...
console.log('   2. Faça upload do arquivo shopify-products-TEST.csv')
console.log('   3. Aguarde a importação')
console.log('   4. Verifique se as imagens foram importadas corretamente')
console.log('\n✨ Se funcionar, rode: npm run generate-csv (para todos os produtos)')
