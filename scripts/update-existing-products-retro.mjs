...
console.log('=== Atualizando Produtos Existentes com RETRO ===\n');
console.log(`Store: ${SHOPIFY_STORE_DOMAIN}`);
console.log(`Admin Token: ${SHOPIFY_ADMIN_TOKEN?.substring(0, 15)}...\n`);

// Load products to update
const productsToUpdate = JSON.parse(fs.readFileSync(
  path.join(__dirname, '..', 'PRODUCTS-TO-ADD-RETRO.json'),
  'utf-8'
));

console.log(`📋 ${productsToUpdate.length} produtos para atualizar\n`);
...
async function processUpdates() {
  ...
  const batchSize = 5; // Update 5 products at a time
  const delay = 500; // 500ms delay between batches

  for (let i = 0; i < productsToUpdate.length; i += batchSize) {
    const batch = productsToUpdate.slice(i, i + batchSize);
    console.log(`\n📦 Processando batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(productsToUpdate.length / batchSize)}...`);
...
// Save results
const resultsPath = path.join(__dirname, '..', 'RETRO-UPDATE-RESULTS.json');
...
console.log(`\n✅ Resultados salvos em: RETRO-UPDATE-RESULTS.json`);
