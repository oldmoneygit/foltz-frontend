#!/usr/bin/env node
/**
 * Script para testar a criação de variantes usando a API correta
 */
...
console.log('🔧 Testando criação de variantes (v2)')
...
async function main() {
  ...
  // 2. Try using productVariantsBulkCreate
  console.log('\n🔧 Tentando adicionar variante 3XL usando productVariantsBulkCreate...')

  const price = targetProduct.variants.edges[0].node.price
  const sizeOption = targetProduct.options.find(o => o.name === 'Size')

  if (!sizeOption) {
    console.log('❌ Produto não tem opção Size')
    return
  }

  // The strategy array defines the variant combination
  const mutation = `
  ...
  if (mutationResult.data?.productVariantsBulkCreate?.userErrors?.length > 0) {
    console.log('\n❌ Erros encontrados:')
    mutationResult.data.productVariantsBulkCreate.userErrors.forEach(err => {
      console.log(`  - ${err.field}: ${err.message}`)
    })
  } else if (mutationResult.data?.productVariantsBulkCreate?.productVariants?.length > 0) {
    console.log('\n✅ Variante criada com sucesso!')
    mutationResult.data.productVariantsBulkCreate.productVariants.forEach(v => {
      console.log('   ID:', v.id)
